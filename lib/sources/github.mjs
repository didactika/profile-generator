...
export class GitHubClient {
  ...
  constructor({ token, log, userAgent = "profile-generator" }) {
    this.#token = token;
    this.#log = log;
  ...
  async get(path, { retries202 = 5, optional = false } = {}) {
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      // A 200 can still carry an empty body — GitHub does this for e.g. the
      // contributors and commit-activity endpoints on a repository that has
      // no commits yet — and res.json() throws on an empty body rather than
      // returning null.
