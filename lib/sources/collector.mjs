...
export class ProfileCollector {
  ...
  async #repositories(login) {
    ...
    const kept = all.filter((r) => !r.archived && !r.fork && !ProfileCollector.#INFRASTRUCTURE.has(r.name));
    ...
    for (const repo of kept) {
      const branches = (await this.#github.get(`/repos/${login}/${repo.name}/branches?per_page=100`)) || [];
