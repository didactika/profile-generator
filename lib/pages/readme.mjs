...
${this.content.sectionEnabled("parent") ? this.#parent(locale) : ""}
...
<p><sub>${escapeXml(f.bio[locale])}</sub></p>
  ...
  #parent(locale) {
    ...
    if (!parent) return "";
    return `<p align="center"><sub>${escapeXml(parent.label[locale])} · <a href="${parent.url}"><strong>${escapeXml(parent.name)}</strong></a></sub></p>`;
  }
