...
<p><sub>${escapeXml(f.bio[locale])}</sub></p>
  ...
  #relationships(locale) {
    ...
    const links = this.content.relationships.map((relationship) =>
      `${escapeXml(relationship.label[locale])} · <a href="${relationship.url}"><strong>${escapeXml(relationship.name)}</strong></a>`
    ).join(" &nbsp;·&nbsp; ");
    return `<p align="center"><sub>${links}</sub></p>`;
  }
