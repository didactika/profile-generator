...
export class ReadmePage {
  ...
  render() {
    ...
    const sections = [
      ...
      this.content.sectionEnabled("donations") && this.#donations(c, locale),
...
`;
  ...
  #donations(c, locale) {
    ...
    const url = this.content.section("donations").url;

    if (!donations?.heading?.[locale] || !url) return "";

    return `## ${donations.heading[locale]}

  ${donations.body[locale]}

  <p align="center">
    <a href="${url}"><strong>☕ ${donations.button[locale]}</strong></a>
  </p>`;
  }
...
<p><sub>${escapeXml(f.bio[locale])}</sub></p>
  ...
  #parent(locale) {
    ...
    return `<br>${this.links.parentAffiliation(locale, label, parent.url)}`;
  }
