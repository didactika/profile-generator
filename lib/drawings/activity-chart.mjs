/** Weekly commit activity across the organisation, over the last 52 weeks, as a
...
export class ActivityChart extends Drawing {
  constructor(theme, timeline, locale, strings, title = "") {
    super(theme);
...
${this.text(W / 2, m.top + 24, this.strings.noData, { size: 12, fill: th.muted, anchor: "middle" })}</g>`);
    }
...
${this.text(m.left - 8, Y(v) + 4, String(Math.round(v)), { size: 10, fill: th.muted, anchor: "end" })}`
  ...
  #monthLabels(X, H) {
    ...
    return Array.from({ length: TICKS }, (_, k) => {
      ...
      const label = month.format(new Date(this.timeline[i].week * 1000)).replace(".", "");
      return this.text(X(i), H - 8, label, { size: 10, fill: this.theme.muted, anchor });
    }).join("\n");
  }
}
