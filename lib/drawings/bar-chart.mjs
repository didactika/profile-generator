...
export class BarChart extends Drawing {
   ...
   * @param strings  {noData, other} — the two words this chart draws that come
   *                 from the profile's language, not from content.json
   ...
   */
  constructor(theme, items, strings, {
...
${this.text(W / 2, top + 20, this.strings.noData, { size: 12, fill: this.theme.muted, anchor: "middle" })}</g>`);
  }
...
<g class="fade" style="${d}">${this.text(W, mid + 4, this.#value(item, total), { size: 11, fill: th.secondary, anchor: "end" })}</g>`;
  ...
  /** Labels are clipped to what actually fits: a long name at 11px would
  ...
  #label(item) {
    return item.name === "__other__" ? this.strings.other : item.name;
  }
