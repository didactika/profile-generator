...
export class Content {
  ...
  get parent() {
    return this.#data.org.parent;
  }

  get memberOrganizations() {
    return (this.#data.org.memberOrganizations || []).map((org) =>
      typeof org === "string" ? { login: org } : org
    );
  }
  ...
  get metricOrganizations() {
    ...
    return [...new Set([
      ...
      ...this.memberOrganizations.map((org) => org.login),
    ].filter(Boolean))];
  }
  ...
  get groups() {
    return (this.#data.groups || []).filter((group) => group.enabled !== false);
  }
  ...
  section(name) {
    return { enabled: true, ...(this.#data.sections?.[name] || {}) };
  }

  sectionEnabled(name) {
    return this.section(name).enabled !== false;
  }

  /** Projects are selected from live repository metadata. Entries in
   *  content.json enrich a project or announce one that is not public yet;
   *  they are not the allowlist for public repositories. */
  projectsIn(groupId, repos = []) {
    const group = this.groups.find((candidate) => candidate.id === groupId);
    const allProjects = this.#data.projects || [];
    const declared = allProjects.filter((project) => project.group === groupId);
    const discoversTypes = Boolean(group?.projectTypes?.length);
    const discoversNpm = group?.discover?.npm === true;
    if (!discoversTypes && !discoversNpm) return declared;

    const liveDeclared = new Set();
    const discovered = repos
      .filter((repo) =>
        (discoversTypes && group.projectTypes.includes(repo.custom_properties?.["project-type"]))
        || (discoversNpm && Boolean(repo.packages?.npm)))
      .filter((repo) => {
        const override = allProjects.find((project) => this.#matches(repo, project.repo));
  ...
  #projectFrom(repo, groupId) {
    ...
    const npm = repo.packages?.npm?.name
      || (npmMatch ? decodeURIComponent(npmMatch[1]) : undefined);
