...
import { ParentAffiliation } from "./drawings/parent-affiliation.mjs";
...
export class ProfileGenerator {
  ...
  async #drawAssets() {
    ...
    for (const mode of MODES) {
      const theme = new Theme(mode, this.content.theme);
      await this.#drawTabs(theme, mode);
      if (this.content.sectionEnabled("parent") && this.content.parent) {
        await this.#drawParentAffiliations(theme, mode);
      }
      if (this.content.sectionEnabled("metrics")) {
  ...
  /** One fixed-size parent label per locale and colour scheme. */
  async #drawParentAffiliations(theme, mode) {
    const parent = this.content.parent;
    for (const locale of LOCALES) {
      const label = `${parent.label[locale]} · ${parent.name}`;
      const drawing = new ParentAffiliation(theme, label);
      await this.assets.write(`parent-${locale}-${mode}.svg`, drawing.render());
    }
  }
  ...
  async #renderPages() {
    ...
    for (const locale of LOCALES) {
      ...
      if (this.content.sectionEnabled("projects")) {
